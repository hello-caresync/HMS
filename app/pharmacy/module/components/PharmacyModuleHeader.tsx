...
type PharmacyModuleHeaderProps = {
  lookupValue: string;
  onLookupChange: (value: string) => void;
  resultCount?: number;
};

export default function PharmacyModuleHeader({ lookupValue, onLookupChange, resultCount }: PharmacyModuleHeaderProps) {
  return (
    <header className="shrink-0 border-b border-[#E2E8F0] bg-white px-4 py-2">
      <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-base font-bold text-[#0F172A]">Pharmacy Management (HPMS)</h1>
          <p className="text-[9px] text-slate-500">Dispensing · inventory · FEFO · procurement · controlled drugs · billing</p>
        </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search pharmacy — RX #, UHID, patient, or medicine name"
