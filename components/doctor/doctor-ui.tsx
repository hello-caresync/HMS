...
export function ClinicalPageHeader({
  ...
  return (
    <div className="mb-4 flex flex-wrap items-start justify-between gap-3 border-b border-slate-200/80 pb-4">
      <div>
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {items.map((v) => (
        ...
        >
          ...
          <p className={`mt-1 text-2xl font-bold tabular-nums ${v.critical ? 'text-[#EF4444]' : 'text-[#0D9488]'}`}>
            {v.value}
            {v.unit && <span className="ml-1 text-sm font-medium text-[#64748B]">{v.unit}</span>}
          </p>
        </div>
      ))}
    </div>
  ...
  return (
    <header className={`${clinicalClasses.card} p-4`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold text-[#0F172A]">{props.name}</h2>
        ...
        <div className="flex flex-wrap gap-1.5">
          {props.allergies.length === 0 ? (
            <span className="rounded-full bg-[#10B981]/10 px-2 py-0.5 text-xs font-semibold text-[#0D9488]">NKDA</span>
          ) : (
  ...
  return (
    <div className="relative">
      <input
        ...
        className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-[#0D9488] focus:outline-none focus:ring-2 focus:ring-[#0D9488]/30"
      />
        ...
        <ul className="absolute z-20 mt-1 max-h-48 w-full overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-lg">
          {filtered.slice(0, 8).map((o) => (
            <li key={o.code}>
              ...
              >
                <span className="font-mono font-semibold text-[#0D9488]">{o.code}</span> — {o.label}
              </button>
            </li>
          ))}
        </ul>
...
export function CriticalAlertBanner({ messages }: { messages: string[] }) {
  ...
  return (
    <div className="animate-pulse rounded-xl border border-[#EF4444]/30 border-l-4 border-l-[#EF4444] bg-red-50 p-4">
      ...
      <ul className="mt-2 space-y-1 text-sm font-semibold text-[#0F172A]">
        {messages.map((m, i) => (
          <li key={i}>{m}</li>
        ))}
      </ul>
...
export function DigitalSignaturePad({ onApply }: { onApply: (token: string) => void }) {
  return (
    <div className="rounded-xl border border-dashed border-slate-300 bg-[#F8FAFC] p-4 text-center">
      <p className="text-sm text-[#64748B]">Digital signature canvas · HIPAA audit logged</p>
