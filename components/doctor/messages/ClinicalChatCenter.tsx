    ...
    <div className={`${clinicalClasses.pageBg} flex h-[calc(100vh-6rem)] flex-col`}>
      ...
      <div className="flex min-h-0 flex-1 gap-3 overflow-hidden">
          ...
          {channels.map((ch) => (
            <button
              ...
              className={`mb-1 w-full rounded-lg px-3 py-2 text-left text-sm ${
                channelId === ch.id ? 'bg-brand/10 font-semibold' : 'hover:bg-brand-light/40'
              }`}
            >
        ...
        <div className={`${clinicalClasses.card} flex min-h-0 flex-1 flex-col p-0`}>
          <div className="border-b bg-brand-text px-4 py-2 text-sm font-semibold text-brand-surface">{activeChannel?.name ?? 'Channel'}</div>
          <ul className="flex-1 space-y-2 overflow-y-auto p-4">
            {thread.map((m) => (
              <li key={m.id} className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${m.sender === 'You' ? 'ml-auto bg-brand text-white' : 'bg-brand-light/50'}`}>
                {m.stat && (
